...
class TestEstimatingEquations:
    ...
    def test_cox_ph(self):
        ...
        d['C'] = np.exp(2.72811 - np.log(1.5) * d['z']) * np.random.exponential(size=n)
        ...
        mestimator = MEstimator(psi_coxph, init=[0., 0., 0.])
        mestimator.estimate()

        # Comparing to statsmodels PHReg
        coxph = sm.PHReg(d['t'], d[['x', 'w', 'z']], status=d['d']).fit()

        # Checking mean estimate
        npt.assert_allclose(mestimator.theta,
                            np.asarray(coxph.params),
                            atol=1e-6)

        # Checking variance estimates
        # TODO need to find appropriate variance comparison
    ...
    def test_kaplan_meier(self):
        ...
        def psi_km(theta):
            return ee_kaplan_meier(theta,
                                   time=d['t'],
                                   delta=d['d'])

        initials = [0.5, ] * np.unique(d.loc[d['d'] == 1, 't']).shape[0]
        mestimator = MEstimator(psi_km, init=initials)
        mestimator.estimate()

        # Checking against statsmodels Kaplan-Meier
        km = sm.SurvfuncRight(time=d['t'], status=d['d'])

        # Checking mean estimate
        npt.assert_allclose(mestimator.theta,
                            np.asarray(1 - km.surv_prob),
                            atol=1e-6)
